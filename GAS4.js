...
d3.json('/GAS').then(response => {
    ...
    // --- SVG Setup ---
    const width = 800, height = 400, margin = { top: 90, right: 40, bottom: 50, left: 60 };
    const svg = d3.select("body").append("svg")
        .attr("width", width + margin.left + margin.right + 200) // Increased width to accommodate legend
    ...
    function updateChart(keyIndex) {
        ...
        yAxisGroup.call(d3.axisLeft(yScale).tickFormat(d3.format(".2f")));
    ...
    const legend = svg.append("g")
        .attr("class", "legend")
        .attr("transform", `translate(${width + margin.right}, ${margin.top})`); // Adjusted transform
