d3.json('/GAS').then(response => {
  ...
  svg.append("text") // y-axis label
    ...
    .attr("dy", "1em")
    .attr("text-anchor", "middle")
    .text("Diesel Price (prices in cents per litre)"); 
    const legend = svg.append("g")
    .attr("font-family", "sans-serif")
    .attr("font-size", 10)
    .attr("text-anchor", "end")
    .selectAll("g")
    .data(colorScale.domain()) // Use the domain of your color scale
    .enter().append("g")
    .attr("transform", (d, i) => `translate(0,${i * 20})`); // Position legend items
