...
// Import the necessary functions for linear regression
const regression = require('regression');

// Predict the next 5 years of data based on the existing data
function predictNext5Years(data) {
    // Extract x (year) and y (gasoline price) values from the existing data
    const xyData = data.map(d => [d.date.getFullYear(), d.value]);
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        const predictedPrice = result.predict(year)[1]; // Using index 1 for the predicted value
        predictions.push({ year, value: predictedPrice });
    }
...
d3.json('/GAS').then(response => {
  ...
  nextChartButton.addEventListener('click', () => {
      ...
      const currentData = response[currentIndex];
      updateChart(currentIndex);

      // Predict next 5 years based on current data
      const predictions = predictNext5Years(currentData);
      console.log("Predictions for the next 5 years:", predictions);
  });
});
